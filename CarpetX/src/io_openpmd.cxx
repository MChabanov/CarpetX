...
void OutputOpenPMDPlanes(const cGH *const cctkGH,
  ...
  if (!carpetx_openpmd_t::self)
    carpetx_openpmd_t::self = std::make_optional<carpetx_openpmd_t>();
  carpetx_openpmd_t::self->OutputOpenPMDPlanes(cctkGH, output_group, planes,
    ...
    for (const auto &leveldata : patchdata.leveldata) {
      ...
      for (int gi = 0; gi < numgroups; ++gi) {
        if (input_group.at(gi)) {
          ...
          const int tl_count =
              tl_mode == TimeLevelMode::All ? int(groupdata.mfab.size()) : 1;
          for (int tl = 0; tl < tl_count; ++tl) {
            ...
            for (int local_component = 0;
              ...
              for (int d = 0; d < 3; ++d)
                // assert(start.at(d) >= 0);
                assert(start.at(d) <
                       std::numeric_limits<std::remove_reference_t<decltype(
                               start.at(d))> >::max() /
                           2);
              for (int d = 0; d < 3; ++d)
...
#pragma omp simd
                  ...
                  record_components.at(vi).loadChunkRaw(contig_ptr, start,
                                                        count);
                  tasks.emplace_back([=]() {
...
void carpetx_openpmd_t::OutputOpenPMDPlanes(
    const cGH *const cctkGH, const std::vector<bool> &output_group,
    const std::vector<plane_spec_t> &planes, const std::string &output_dir,
    const std::string &output_file) {
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
